...
var KiteConnect = class {
  constructor(params) {
    ...
    this.enc_token = params.enc_token || null;
    this.default_login_uri = defaults.login;
    this.session_expiry_hook = null;
    this.requestInstance = import_axios.default.create({
      baseURL: this.root,
  ...
  generateSession(request_token, api_secret) {
    return new Promise((resolve, reject) => {
      const checksum = (0, import_node_crypto.createHash)("sha256")
  ...
  validatePostback(postback_data, api_secret) {
    ...
    let checksum;
    try {
      checksum = (0, import_node_crypto.createHash)("sha256")
        .update(inputString)
        .digest("hex");
    } catch (e) {
      throw e;
  ...
  transformInstrumentsResponse(data, headers) {
    if (headers["content-type"] === "text/csv") {
      const parsedData = import_papaparse.default.parse(data, {
        header: true,
      }).data;
  ...
  request(
    ...
    if (method === "GET" || method === "DELETE") {
      ...
      if (isJSON) {
        ...
        payload = import_node_querystring.default.stringify(params);
      }
    }
    const options = {
      method,
    ...
    if (this.access_token) {
      const authHeader = "enctoken " + this.enc_token;
    ...
    if (responseTransformer) {
      if (!import_axios.default.defaults.transformResponse) {
        options.transformResponse = responseTransformer;
      }
      if (Array.isArray(import_axios.default.defaults.transformResponse)) {
        options.transformResponse =
          import_axios.default.defaults.transformResponse.concat(
            responseTransformer
          );
      } else {
        options.transformResponse = [
          import_axios.default.defaults.transformResponse,
...
var KiteTicker = class {
  ...
  constructor(params) {
    ...
    this.enc_token = params.enc_token;
  ...
  connect() {
    if (
      this.ws &&
      (this.ws.readyState == import_ws.default.CONNECTING ||
        this.ws.readyState == import_ws.default.OPEN)
    )
      return;
    ...
    const url =
      ...
      this.user_id +
      "&enctoken=" +
      encodeURIComponent(this.enc_token) +
      "&uid=" +
    ...
    this.ws.onopen = () => {
      ...
      if (this.read_timer)
        (0, import_node_timers.clearInterval)(this.read_timer);
  ...
  disconnect() {
    if (
      this.ws &&
      this.ws.readyState != import_ws.default.CLOSING &&
      this.ws.readyState != import_ws.default.CLOSED
    ) {
      this.ws.close();
    }
  }
